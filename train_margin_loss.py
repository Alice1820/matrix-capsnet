...
from dataset import smallNORB, collate_data
from model_margin_loss import CapsNet, CapsuleLoss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--root', type=str, default="../smallnorb")
    ...
    parser.add_argument('--loss-type', type=str, default='margin_loss', metavar='N', help='loss to use: cross_entropy_loss, margin_loss, spread_loss')
    ...
    if args.multi_gpu:
        ...
        model = nn.parallel.DataParallel(CapsNet(args.batch_size, img_size, A, B, C, D, E, r))
    else:
        model = CapsNet(args.batch_size, img_size, A, B, C, D, E, r)
    ...
    if args.dataset == 'mnist':
        train_loader = DataLoader(dataset=mnist_train_dataset,
                                                   batch_size=args.batch_size,
                                                   num_workers=args.num_workers,
                                                   shuffle=True)

        test_loader = DataLoader(dataset=mnist_test_dataset,
                                                  batch_size=args.batch_size,
                                                  num_workers=args.num_workers,
                                                  shuffle=True)
    if args.dataset == 'smallnorb':
        train_loader = DataLoader(smallNORB(args.root, 'train'),
                                batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers,
                                collate_fn=collate_data, pin_memory=use_cuda)
        test_loader = DataLoader(smallNORB(args.root, 'test'),
                                batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers,
                                collate_fn=collate_data, pin_memory=use_cuda)

    print (args.dataset)
    steps, lambda_, m = len(train_loader), 1e-3, 0.2
    ...
    for epoch in range(args.num_epochs):
        ...
        for data in train_loader:
            step += 1
            if lambda_ < 1:
                lambda_ += 2e-1 / (len(train_loader) * args.batch_size)
            if m < 0.9:
                m += 2e-1 / (len(train_loader) * args.batch_size)

            capsule_loss = CapsuleLoss(m, args.loss_type)
            ...
            if step % args.print_freq == 0:
                print ('epoch: ' + str(epoch) + ' %d/%d' % (step, steps) + ' loss: ' + str(meter_loss.value()[0]) + ' acc: ' + str(meter_accuracy.value()[0]))
