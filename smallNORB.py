...
def write_data_to_tfrecord(kind, chunkify=False):
    ...
    """Read data"""
    if kind == "train":
        fid_images = open('./smallnorb-5x46789x9x18x6x2x96x96-training-dat.mat', 'rb')
        fid_labels = open('./smallnorb-5x46789x9x18x6x2x96x96-training-cat.mat', 'rb')
        dir_name = './smallnorb_train'
    elif kind == "test":
        fid_images = open('./smallnorb-5x01235x9x18x6x2x96x96-testing-dat.mat', 'rb')
        fid_labels = open('./smallnorb-5x01235x9x18x6x2x96x96-testing-cat.mat', 'rb')
        dir_name = './smallnorb_test'
    else:
    ...
    for j in range(total_num_images // CHUNK if chunkify else 1):
        ...
        for i in range(num_images):
            ...
            lab = labels[i]
            try:
                os.mkdir(os.path.join(dir_name, str(lab)))
            except:
                pass
            cv2.imwrite(os.path.join(os.path.join(dir_name, str(lab)), '%d' % lab +'_%d' % i + '.jpg'), img.flatten().reshape(96, 96))
    # pass
...
# def read_norb_tfrecord(filenames, epochs):
#     """Credit: http: // ycszen.github.io / 2016 / 08 / 17 /
#        Original Version: Ycszen
#     """
#
